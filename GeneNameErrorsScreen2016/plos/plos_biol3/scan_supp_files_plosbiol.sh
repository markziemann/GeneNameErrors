...
GENELIST_DIR='/home/mdz/Bioinformatics/supp_scan/genelists/'
...
wget -P $TEMP_DIR --content-disposition "$SUPP_URL"

TEMP_FILE=`find $TEMP_DIR | tail -1`
FILE_SFX=`echo $TEMP_FILE | rev | cut -d '.' -f1 | rev `

#Check the filename in case its a ZIP archive
ZIP=`echo $FILE_SFX | grep -wci zip$`
if [ "$ZIP" -eq "1" ] ; then
...
| parallel -j2 scanit "{}" "$GENELIST_DIR" 2> scan_sterr.txt
